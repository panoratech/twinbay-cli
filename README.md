...
| Run | You get |
|-----|---------|
| `twinbay --help`, `twinbay environments list --help` | Commands by category, runnable examples, flags |
| `twinbay --usage`, `twinbay environments list --usage` | The command surface as machine-readable [KDL](https://kdl.dev): commands, aliases, flags, defaults, env vars, config keys |
...
| `twinbay environments list --dry-run` | The exact HTTP request (method, URL, headers, body), with no credentials or network call |
| `twinbay environments list --output-format json` (or `--jq`) | Machine-readable output |
...
twinbay environments list --usage
```
...
twinbay environments list --dry-run

# Machine preview: compact JSON on stdout and silent stderr
twinbay environments list --dry-run --output-format json
```
...
twinbay environments list --interactive
...
```bash
twinbay --organization-api-key "$CLI_TWINBAY_ORGANIZATION_API_KEY" environments list
```
...
```bash
twinbay --server-url https://custom-api.example.com environments list
```
...
twinbay environments list

# Machine-readable JSON
twinbay environments list --output-format json

# TOON for LLM-friendly compact output
twinbay environments list --output-format toon

# Pipe JSON to jq without using --output-format
twinbay environments list --output-format json | jq '.'
```
...
twinbay environments list --output-format json > output.json 2> error.log
if [ $? -ne 0 ]; then
...
```bash
twinbay environments list --debug
```
