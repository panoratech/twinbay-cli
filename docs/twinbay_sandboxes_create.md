...
```
twinbay sandboxes create [flags]
```

### Examples

```
  twinbay sandboxes create --name <value>
```
...
* `twinbay sandboxes create --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `twinbay sandboxes create --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `twinbay sandboxes create --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
