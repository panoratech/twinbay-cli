...
* `twinbay organizations update --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `twinbay organizations update --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `twinbay organizations update --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
