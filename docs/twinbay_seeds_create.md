...
```
  ...
  -n, --name string     Display name of the seed [required]
  -p, --prompt string   The starting state you want, in your own words. "50 open tickets, half of them aging past 90 days." [required]
      --schema          Print the exact JSON Schema of the request body and exit
  -t, --twin string     Slug of the twin this seed is for [required]
```
...
* `twinbay seeds create --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `twinbay seeds create --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `twinbay seeds create --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
