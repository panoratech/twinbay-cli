...
* `twinbay api-keys create --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `twinbay api-keys create --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `twinbay api-keys create --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
