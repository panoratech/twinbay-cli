...
```
twinbay sandbox-twins advance [flags]
```

### Examples

```
  twinbay sandbox-twins advance --sandbox-id 0e5430b8-577a-40fb-a158-9fc5ad75ab04 --sandbox-twin-id 10a63cd8-60d5-4c91-bbd5-7c17aa12a3d7
```
...
* `twinbay sandbox-twins advance --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `twinbay sandbox-twins advance --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `twinbay sandbox-twins advance --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
