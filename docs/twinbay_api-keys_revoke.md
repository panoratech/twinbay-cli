...
```
  twinbay api-keys revoke --api-key-id 70d05b12-db01-4e93-afb7-c26ecf254345
```
...
* `twinbay api-keys revoke --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `twinbay api-keys revoke --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
